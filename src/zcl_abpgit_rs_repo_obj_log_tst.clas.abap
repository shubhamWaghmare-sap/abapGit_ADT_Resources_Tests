CLASS zcl_abpgit_rs_repo_obj_log_tst DEFINITION
  PUBLIC
  ...
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA lo_log_factory TYPE REF TO if_abapgit_app_log_factory.
    DATA lo_log TYPE REF TO if_abapgit_log.

ENDCLASS.



CLASS ZCL_ABPGIT_RS_REPO_OBJ_LOG_TST IMPLEMENTATION.
  ...
  METHOD get.
    ...
    DATA(ls_requested_content_type) = request->get_inner_rest_request( )->get_header_field( iv_name = if_http_header_fields=>accept ).

    "TODO:  remove case co_content_type_object_v1 after 2005 release
...
*------ Get Application Log Key
        ...
        IF lo_log_factory IS INITIAL.
          lo_log_factory = cl_abapgit_app_log_factory=>get_instance( ).
        ENDIF.

        IF lo_log IS INITIAL.
          lo_log = lo_log_factory->load_single( iv_app_log = lv_app_log_key ).
        ENDIF.
